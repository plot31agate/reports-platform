...
| **Money in** | Every client from the bank statement: paid this year, typical month, cadence, last paid — and a status (on track / late / **gone quiet**) measured against each client's own payment rhythm. The **Retainer book** lives here too: the agreed ongoing retainers (a signed retainer's home — NOT Pipeline). Each entry reconciles against the bank by name — already paying → tracked automatically (with an "agreed vs actually paid" mismatch flag); not yet → it feeds this month's expectations and the cash-flow floor from its start date. When Xero sync runs, each late/quiet client also shows their **outstanding invoices** (number, amount, days overdue) — chase a number, not a hunch. |
...
| **Pipeline** | Work you might still win — agreed vs potential, weighted by stage (an ongoing retainer you've signed belongs in the Retainer book instead). Won work feeds the cash-flow floor automatically, with a strict dedupe order — bank rhythm > retainer book > won pipeline — so a client is never counted twice. |
