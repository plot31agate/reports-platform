...
CONNECTOR_DEFS = [
    {
        ...
        "blurb": "Pulls backlinks, 12-month authority trends, core keyword positions (Site Explorer), technical SEO metrics (Site Audit), the competitor benchmark, and — when the client's Search Console is connected to the Ahrefs project — search data too. The curated issue register stays a manual upload.",
        "agency_fields": [
        ...
        "client_fields": [
            ...
            {"key": "core_keywords", "label": "Core keywords", "type": "textarea",
             ...
             "hint": "The keywords this client is judged on. Positions come from Site Explorer, so nothing needs adding to Rank Tracker first - the report shows them in this order with volume, market and the move since last month, and a keyword the site does not rank for shows as unranked. Leave blank to report the domain's 25 highest-volume keywords instead."},
            {"key": "keyword_country", "label": "Keyword market", "type": "text",
             "placeholder": "optional, e.g. GB",
             "hint": "Optional two-letter country code to read positions in one market only. Leave blank to search every market and report each keyword where it ranks strongest."},
